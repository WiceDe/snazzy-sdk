class SnazzySdk {
  ...
  addAction(action){
    ...
    if(!(action.actionType in this.eventListeners)){
      try {
        this.eventListeners[action.actionTyp] = window.addEventListener(action.actionType, this.handleEvent);
      } catch(e) {
  ...
  executeAction(action, additionalData) {
    ...
    for(var i=0;i<selectorList.length;i++){
      ...
      for(var index in elements) {
        var name = ('name' in elements[index] && elements[index].name.trim() !== '')?
