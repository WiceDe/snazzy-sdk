class SnazzySdk {
  ...
  executeAction(action, additionalData) {
    ...
    if(doSend) {
      this.executeWebhookRequest(data, action.alternateWebhook);
    }
  }
  ...
  executeWebhookRequest(body, alternateWebhook) {
    if(!this.webhookUrl && !alternateWebhook) {
      console.log('You need to create and supply a WebhookUrl for using actions!');
      return false;
    }

    var hookUrl = (alternateWebhook)? alternateWebhook : this.webhookUrl;
