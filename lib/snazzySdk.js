class SnazzySdk {
  constructor(data) {
    if(!data || !data.apiKey) {
      window.console && console.log('No api key specified but required!');
      return false;
    }
  ...
  handleEvent(type, event, arg2, arg3) {
     if(event && event.target) {
       ...
       if(type in this.actions) {
         let length = this.actions[type].length;
         for(i=0;i<length;i+=1) {
           additionalData = (type === 'error')? { error: event, url: arg2, line: arg3} : { event };
  ...
  addAction(action){
    ...
    if(!(action.actionType in this.eventListeners)){
      try {
        this.eventListeners[action.actionTyp] = window.addEventListener(action.actionType, handleEvent);
      } catch(e) {
  ...
  executeAction(action, additionalData) {
    ...
    if(action.beforeSendCallBack) {
      ...
      result = action.beforeSendCallBack(action, data);
      if(result === false){
        doSend = false;
      } else {
        body = result;
