class SnazzySdk {
       ...
       if(type in this.actions) {
         ...
         for(var i=0;i<length;i+=1) {
           ...
           if('closest' in element) {
             console.log('Using closest');
             if(element.closest(this.actions[type][i].boundToSelector)) {
               this.executeAction(this.actions[type][i]);
             }
           } else {
             for(var c=0;c<3; c++) {
               if(element.matches(this.actions[type][i].boundToSelector)){
                 this.executeAction(this.actions[type][i]);
                 break;
               }
  ...
  addAction(action){
    ...
    if(!(action.actionType in this.actions)) {
    ...
    if(!(action.actionType in this.eventListeners)){
      ...
      try {
        var that = this;
        if(!this.eventListeners[action.actionType]) this.eventListeners[action.actionType] = [];
        this.eventListeners[action.actionType] = window.addEventListener(action.actionType, function(event) {that.handleEvent(action.actionType, event);});
      } catch(e) {
        if(window.console) console.log(e);
      }

      console.log(this.eventListeners[action.actionType]);
    }
  }
...
if(window.snazzySdk && !window.snazzySdk.send) {
  var sdkSetup = window.snazzySdk;
  window.snazzySdk = new SnazzySdk(sdkSetup.settings);
  if(sdkSetup.callback) sdkSetup.callback();
} else {
